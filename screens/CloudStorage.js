...
import * as docActions from "../store/actions/documents";
...
// }
  ...
  const dataURL = `${database64}`;
    const storageRef = storage().ref('Documents'+`/${userId}/`+ docname);
    //const data = path;
    ...
    task.then((imageSnapshot) => {
      ...
      storage()
        ...
        .then((downloadURL) => {
          ...
          database().ref(`Documents/${userId}`).update({
  ...
  const getAllImages = () => {
    const ref = storage().ref('Documents'+`/${userId}/`);
